...
CREATE TABLE IF NOT EXISTS encounter_preview (
            id INTEGER PRIMARY KEY,
            fight_start INTEGER,
            current_boss TEXT,
            duration INTEGER,
...
CREATE INDEX IF NOT EXISTS encounter_preview_favorite_index ON encounter_preview(favorite);

CREATE INDEX IF NOT EXISTS encounter_preview_fight_start_index ON encounter_preview(fight_start);

CREATE INDEX IF NOT EXISTS encounter_preview_my_dps_index ON encounter_preview(my_dps);

CREATE INDEX IF NOT EXISTS encounter_preview_duration_index ON encounter_preview(duration);

CREATE VIRTUAL TABLE IF NOT EXISTS encounter_search USING fts5(
        current_boss,
        players,
        columnsize = 0,
        detail = full,
...
CREATE TRIGGER IF NOT EXISTS encounter_preview_ai AFTER INSERT ON encounter_preview BEGIN
            INSERT INTO encounter_search(rowid, current_boss, players)
            VALUES (new.id, new.current_boss, new.players);
        END;

CREATE TRIGGER IF NOT EXISTS encounter_preview_ad AFTER DELETE ON encounter_preview BEGIN
...
CREATE TRIGGER IF NOT EXISTS encounter_preview_au AFTER UPDATE OF current_boss, players ON encounter_preview BEGIN
            INSERT INTO encounter_search(encounter_search, rowid, current_boss, players)
            VALUES('delete', old.id, old.current_boss, old.players);
            INSERT INTO encounter_search(rowid, current_boss, players)
            VALUES (new.id, new.current_boss, new.players);
